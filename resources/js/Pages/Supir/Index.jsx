...
export default function SupirIndex({ supirs, auth }) {
    const handleDelete = (id) => {
        if (confirm('Apakah Anda yakin ingin menghapus supir ini?')) {
            axios.delete(route('supirs.destroy', id))
                .then(() => {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Daftar Supir</h2>}
        >
            <Head title="Supir" />
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white shadow-lg rounded-lg overflow-hidden">
                        <div className="p-6 bg-gray-100 border-b border-gray-200">
                            <h3 className="text-2xl font-bold text-gray-800 mb-4">Daftar Supir</h3>

                            {/* Add New Supir Button */}
                            <div className="mb-4">
                                <Link
                                    href={route('supirs.create')}
                                        ...
                                        {supirs.map((supir) => (
                                            <tr key={supir.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-4 py-2 whitespace-nowrap text-sm flex space-x-2">
                                                    {/* Edit Button */}
                                                    <Link
                                                        href={route('supirs.edit', supir.id)}
